...
export function findQuestion(
    ...
    const found = deepCopy.find((question) => question.id === id);
...
export function removeQuestion(questions: Question[], id: number): Question[] {
    ...
    const arr = newArray.filter((questions) => questions.id !== id);
    return arr;
}
...
export function duplicateQuestionInArray(
    ...
    // If not found, return original array (or handle as needed)
    if (index === -1) {
        return [...questions];
    }

    const originalQuestion = questions[index];
    const duplicated = duplicateQuestion(newId, originalQuestion);

    // Create new array with duplicate inserted right after the original
    return [
